...
def create_group(speaker, global_mates):
    if len(global_mates) >= speaker.model.max_groups:
        return
    mates = speaker.model.grid.get_neighbors(speaker.pos, moore=True, include_center=True)
    to_exit = True
    for mate in mates:
